...
- `avatar_url` is used only when it is a non-empty `mxc://` URI. Hermes preserves the configured URI exactly and treats it as opaque: it does not fetch, upload, inspect, convert, or proxy media.
