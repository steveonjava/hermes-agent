    ...
    async def _sync_loop(self) -> None:
        ...
        while not self._closing:
            try:
                ...
                # Treat a non-dict sync result as permanent only when the classifier says so.
                _sync_msg = getattr(sync_data, "message", None)
                if _sync_msg and isinstance(_sync_msg, str):
                    _structured = isinstance(getattr(sync_data, "errcode", None), str) or isinstance(
                        getattr(sync_data, "http_status", None), int
                    )
                    _permanent = _is_permanent_matrix_auth_error(sync_data if _structured else _sync_msg)
                    if _permanent:
